...
userNameInput.addEventListener(
  ...
  (event) => {
    if(event.key === 'Enter') {
      assessmentButton.dispatchEvent(new Event('click'));  // dispatchEvent : イベントを自分で発生させる
    }
  }
)
...
assessmentButton.addEventListener(
  ...
  () => {
    ...
    const hrefValue =   // a要素のhref属性に設定するためのURLを変数に代入している。
      'https://twitter.com/intent/tweet?button_hashtag=' +
      encodeURIComponent("あなたのいいところ") + 
      "&ref_src=twsrc%5Etfw";


    // setAttribute = HTML要素に属性を追加・変更するメソッド。
    // 例えば、<a href="https://example.com" class="btn">リンク</a> 「href=...」,「class=...」が属性
